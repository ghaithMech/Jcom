...
/**
 * REST controller for managing {@link fr.acensi.jcom.domain.Skill}.
 */
@RestController
@RequestMapping("/api")
public class SkillResource {
     ...
     */
    @GetMapping("/skills")
    public ResponseEntity<List<Skill>> getAllSkills(Pageable pageable, @RequestParam(required = false, defaultValue = "false") boolean eagerload) {
        log.debug("REST request to get a page of Skills");
        Page<Skill> page;
        if (eagerload) {
            page = skillService.findAllWithEagerRelationships(pageable);
        } else {
            page = skillService.findAll(pageable);
        }
